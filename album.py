...
def get_album_stats(input_txt='album.txt'):
    ...
    with open(input_txt, 'r') as w:
        ...
        for line in lines:
            al_array=line.split('\t')
            album_id = str(al_array[0])
            #album_name=str(al_array[2])
            play_times=al_array[1].strip()
            original_album_set.add(album_id)

            if not play_times.isdigit() :
                logging.warning('Warnning: play time not number')
            ret = db_instance.execute('''select al.list_id, al.list_name, al.artist_id,  
                                            so.artist_name, so.song_id, so.song_name,
                                            al.tag_xiaomi 
                                         from t_list as al, t_song as so 
                                         where al.list_id = %s and 
                                             al.list_type ='album' and 
                                             al.list_id = so.album_id
                                      '''% (album_id))
            ...
            for i in range(len(ret)):
                ...
                if data_file['list_id'] not in stored_id:
                    data_line.append([data_file['list_id'], data_file['list_name'], str(play_times),
                                      data_file['artist_id'], data_file['artist_name'], data_file['song_id'],
                                      data_file['song_name']] + insert_tag[0] + insert_tag[1])
                    stored_id.add(data_file['list_id'])
                    stored_name.add(data_file['list_name'])

                    if data_file['list_name'] in stored_name:
                        logging.warning('same name album: %s' % (data_file['list_name']))
                        same_line.append([data_file['list_id'], data_file['list_name'], str(play_times),
                                          data_file['artist_id'], data_file['artist_name'], data_file['song_id'],
                                          data_file['song_name']] + insert_tag[0] + insert_tag[1])

    checkmissing(original_album_set,stored_id)
    db_instance.close()
